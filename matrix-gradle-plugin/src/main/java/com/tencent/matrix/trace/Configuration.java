...
public class Configuration {
    ...
    private HashSet<String> blackClasses = new HashSet<>();
    private HashSet<String> blackPackages = new HashSet<>();
        ...
        if (blackArray != null) {
            for (String black : blackArray) {
                ...
                if (black.startsWith("-keepclass ")) {
                    ...
                    blackClasses.add(processor.proguardClassName(black, black));

                } else if (black.startsWith("-keeppackage ")) {
                    black = black.replace("-keeppackage ", "");
                    blackPackages.add(processor.proguardPackageName(black, black));
                }
            }
        }
        return blackClasses.size() + blackPackages.size();
    }

    boolean isInBlackList(String clsName) {
        if (blackClasses.contains(clsName)) {
            return true;
        }

        for (String blackPackage : blackPackages) {
            if (clsName.startsWith(blackPackage)) {
                return true;
            }
        }
